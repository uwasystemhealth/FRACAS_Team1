...
from api.serializers import (UserSerializer, TeamSerializer, SubsystemSerializer, 
                                  RecordSerializer, CommentSerializer)
...
class UserSerializerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.serializer = UserSerializer(instance=self.user, context={'request': request})
...
class TeamSerializerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.serializer = TeamSerializer(instance=self.team, context={'request': request})
...
class SubsystemSerializerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.serializer = SubsystemSerializer(instance=self.subsystem, context={'request': request})
...
class RecordSerializerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.serializer = RecordSerializer(instance=self.record, context={'request': request})
...
class CommentSerializerTestCase(TestCase):
    ...
    def setUp(self):
        self.factory = APIRequestFactory()
        request = self.factory.get('/')
        self.user = User.objects.create(email='user@example.com', first_name='John', last_name='Doe')  # Assume User model is already created.
